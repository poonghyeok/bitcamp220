...
public class BoardDAO {
	...
	public BoardDAO() {
		...
		try {
			Reader reader = Resources.getResourceAsReader("mybatis-config.xml"); // myBatis-config.xml로 부터 일단 읽어오고
																					// sqlSessionFactory에 꽂아준다./
	...
	public List<BoardDTO> getBoardById(Map<String, String> searchMap) {
		List<BoardDTO> boardList = new ArrayList<>();
		SqlSession sqlSession = sqlSessionFactory.openSession();
		boardList = sqlSession.selectList("boardSQL.getBoardById", searchMap);
		sqlSession.close();
		
		return boardList;
	}
	
	public List<BoardDTO> getBoardBySubject(Map<String, String> searchMap) {
		List<BoardDTO> boardList = new ArrayList<>();
		SqlSession sqlSession = sqlSessionFactory.openSession();
		boardList = sqlSession.selectList("boardSQL.getBoardBySubject", searchMap);
		sqlSession.close();
	...
	public int getTotalSearchById(String id) {
		int totalSearchNum = 0;
		//@@@@여기서 부터@@@@@@
		SqlSession sqlSession = sqlSessionFactory.openSession();
		totalSearchNum = sqlSession.selectOne("boardSQL.getTotalSearchById", id);

		return totalSearchNum;
	
	}
	public int getTotalSearchBySubject(String subject) {
		int totalSearchNum = 0;
		//@@@@여기서 부터@@@@@@
		SqlSession sqlSession = sqlSessionFactory.openSession();
		totalSearchNum = sqlSession.selectOne("boardSQL.getTotalSearchBySubject", subject);
...
public void increaseBoardHit(int seq) {
	...
	public int boardDelte(Map<String, Integer> map) {
		...
		sqlSession.delete("boardSQL.deleteAtOnce", map);
		/*
		 * if(map.get("pseq") != map.get("seq")) { 답글이 삭제되는 경우
		 * sqlSession.update("boardSQL.deleteReplyDecrease", map);
		 * sqlSession.update("boardSQL.deleteReplySubject", map); delNum =
		 * sqlSession.delete("boardSQL.deleteBoard", map); }else{ 원글이 삭제되는 경우
		 * sqlSession.update("boardSQL.deleteReplySubject", map); delNum =
		 * sqlSession.delete("boardSQL.deleteBoard", map); }
		 */
	 ...
	 * rs.getMetaData(); dataTrans(list, tableMeta, rs, rsmd); * } catch
	 * (SQLException e) { e.printStackTrace(); } finally { allClose(); }
	 * 
	 * return this.list; }
	 * 
